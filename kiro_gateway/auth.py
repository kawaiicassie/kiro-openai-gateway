...
class KiroAuthManager:
    ...
    def __init__(
        ...
        Args:
            ...
            creds_json: JSON string with credentials (optional, for VPS deployment)
        """
        ...
        # Load credentials from JSON string if specified (priority for VPS deployment)
        if creds_json:
            self._load_credentials_from_json_string(creds_json)
        # Load credentials from file if specified
        elif creds_file:
            self._load_credentials_from_file(creds_file)
    
    def _load_credentials_from_json_string(self, json_string: str) -> None:
        """
        Loads credentials from a JSON string (for VPS deployment).
        
        This method is useful when deploying to platforms like Zeabur, Railway, etc.
        where you cannot upload files but can set environment variables.
        ...
        Args:
            json_string: JSON string containing credentials
        """
        try:
            ...
            logger.info("Credentials loaded from KIRO_CREDS_JSON environment variable")
            
        except json.JSONDecodeError as e:
            logger.error(f"Error parsing KIRO_CREDS_JSON: Invalid JSON format - {e}")
        except Exception as e:
            logger.error(f"Error loading credentials from JSON string: {e}")
